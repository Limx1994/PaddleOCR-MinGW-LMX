...
// Safe wrapper for cv::imwrite that catches OpenCV exceptions.
// This avoids terminate-handler crashes when the OpenCV imgcodecs
// codec registry is empty (e.g. due to static-init issues in MinGW
// --whole-archive builds).
static inline bool SafeImwrite(const std::string &path,
                               const cv::Mat &img,
                               const std::vector<int> &params = {}) {
  try {
    return cv::imwrite(path, img, params);
  } catch (const cv::Exception &e) {
    fprintf(stderr, "[WARN] cv::imwrite failed for '%s': %s\n",
            path.c_str(), e.what());
    return false;
    ...
    fprintf(stderr, "[WARN] cv::imwrite failed for '%s': unknown error\n",
            path.c_str());
    return false;
  }
}
