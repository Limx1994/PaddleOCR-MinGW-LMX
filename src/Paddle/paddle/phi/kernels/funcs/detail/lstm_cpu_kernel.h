...
// Note: Cannot define operator+= for __m256 type (built-in type)
// Use _mm256_add_ps directly instead
#endif
#endif
  ...
  for (int i = 0; i < frame_size / 8; i++) {
    ...
    if (value.prev_state_value) {
      if (grad.check_ig_grad)
        (reinterpret_cast<__m256 *>(grad.check_ig_grad))[i] = _mm256_add_ps((reinterpret_cast<__m256 *>(grad.check_ig_grad))[i], r_checkIGrad);
      if (grad.check_fg_grad)
        (reinterpret_cast<__m256 *>(grad.check_fg_grad))[i] = _mm256_add_ps((reinterpret_cast<__m256 *>(grad.check_fg_grad))[i], r_checkFGrad);
    }
    if (grad.check_og_grad)
      (reinterpret_cast<__m256 *>(grad.check_og_grad))[i] = _mm256_add_ps((reinterpret_cast<__m256 *>(grad.check_og_grad))[i], r_checkOGrad);
  }
#endif
}
