...
// Helper to dispatch reduce based on output dimensionality
template <typename Context, typename T, size_t D, size_t R_D, typename Functor,
          typename XType, typename ReduceDimType>
void ReduceFunctorImpl(
    const Eigen::DefaultDevice& place,
    XType& x,
    phi::DenseTensor* output,
    const DDim& out_dims,
    const ReduceDimType& reduce_dim,
    Functor& functor,
    typename std::enable_if<(D == R_D)>::type* = nullptr) {
...
void ReduceFunctor(const Context& context,
  ...
  ReduceFunctorImpl<Context, T, D, R_D, Functor>(
      place, x, output, out_dims, reduce_dim, functor);
}
