...
1. Build different binaries per release to satisfy different environments:
    - Binaries for different CUDA and cuDNN versions, like CUDA 11.x, 12.x
...
| `WITH_MKL` | ON | Build with [Intel® MKL](https://software.intel.com/en-us/mkl) and [oneDNN](https://github.com/oneapi-src/oneDNN) support. |
...
| `PYTHON_ABI` | "" | Build for different Python ABI support, e.g. cp39-cp39, cp310-cp310 |
...
you can choose either latest or images with a release tag like `3.0.0`,
...
| 3.0.0 | release 3.0.0 CPU only binary image |
| 3.0.0-gpu | release 3.0.0 with GPU support |
...
ADD python/dist/paddlepaddle-3.0.0-cp39-cp39-linux_x86_64.whl /
RUN pip install /paddlepaddle-3.0.0-cp39-cp39-linux_x86_64.whl && rm -f /*.whl
```
