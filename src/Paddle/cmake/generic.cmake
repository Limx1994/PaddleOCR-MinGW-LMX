...
function(merge_static_libs TARGET_NAME)
  ...
  if(LINUX)
    ...
    # Use file(GENERATE) to create MRI script with generator expressions
  ...
  # MinGW GCC: use "ar" with MRI script to merge archives (same as Linux)
  if(MINGW)
    set(mri_file
        ${CMAKE_CURRENT_BINARY_DIR}/${TARGET_NAME}.mri
        CACHE INTERNAL "merge mri file")
    set(merge_script
        ${CMAKE_CURRENT_BINARY_DIR}/${TARGET_NAME}_merge.bat
        CACHE INTERNAL "merge script")
    # Use file(GENERATE) to create MRI script with generator expressions
    set(mri_content "create $<TARGET_FILE:${TARGET_NAME}>\n")
    foreach(lib ${libs})
      string(APPEND mri_content "addlib $<TARGET_FILE:${lib}>\n")
    ...
    string(APPEND mri_content "save\nend\n")
    file(GENERATE OUTPUT ${mri_file} CONTENT ${mri_content})
    # Create batch script to run ar -M
    file(WRITE ${merge_script} "@echo off\n${CMAKE_AR} -M < ${mri_file}\n")
    add_custom_command(
      TARGET ${TARGET_NAME}
      POST_BUILD
      COMMENT "Merge and generate static lib: lib${TARGET_NAME}.a"
      COMMAND cmd /C ${merge_script}
      COMMAND ${CMAKE_RANLIB} "$<TARGET_FILE:${TARGET_NAME}>")
  elseif(WIN32)
    # MSVC: use 'lib.exe' to merge libs
    foreach(lib ${libs})
      set(libfiles ${libfiles} $<TARGET_FILE:${lib}>)
    endforeach()
    add_custom_command(
